...
class UniMolInferLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # src_tokens = sample[input_key]["src_tokens"].ne(self.padding_idx)
        src_tokens = sample[target_key]["tokens_atomic"].ne(self.padding_idx)
        (
            encoder_rep,
            encoder_pair_rep,
            decoder_pair_rep_topo, 
            decoder_pair_rep_geom
        ) = model(**sample[input_key], features_only=True, encoder_masked_tokens=masked_tokens, all_atomic_tokens=all_atomic_tokens)
        sample_size = sample[input_key]["src_tokens"].size(0)
        encoder_pair_rep_list = []
        decoder_pair_rep_topo_list, decoder_pair_rep_geom_list = [],[]
        shortest_list, distance_list = [], []
        atoms_list = []
        if 'pdb_id' in sample[target_key].keys():
            name_key = 'pdb_id'
        elif 'smi_name' in sample[target_key].keys():
        ...
        for i in range(sample_size):  # rm padding token
            encoder_pair_rep_list.append(encoder_pair_rep[i][src_tokens[i], :][:, src_tokens[i]].data.cpu().numpy())
            decoder_pair_rep_topo_list.append(decoder_pair_rep_topo[i][src_tokens[i], :][:, src_tokens[i]].data.cpu().numpy())
            decoder_pair_rep_geom_list.append(decoder_pair_rep_geom[i][src_tokens[i], :][:, src_tokens[i]].data.cpu().numpy())
            shortest_list.append(sample[target_key]["shortest_target"][i][src_tokens[i], :][:, src_tokens[i]].data.cpu().numpy())
            distance_list.append(sample[target_key]["distance_target"][i][src_tokens[i], :][:, src_tokens[i]].data.cpu().numpy())
            atoms_list.append(sample[target_key]["tokens_atomic"][i][src_tokens[i]])
        logging_output = {
                "mol_repr_cls": encoder_rep[:, 0, :].data.cpu().numpy(),  # get cls token
                "pair_repr": encoder_pair_rep_list,
                "decoder_topo_pair_repr": decoder_pair_rep_topo_list,
                "decoder_geom_pair_repr": decoder_pair_rep_geom_list,
                "shortest": shortest_list,
...
@register_loss("unimol_finetune_infer")
class UniMolFinetuneInferLoss(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()
    ...
    def forward(self, model, sample, reduce=True):
        ...
        sample_size = sample[input_key]["src_tokens"].size(0)
        encoder_pair_rep_list = []
        if "finetune_target" in sample[target_key].keys():
            target_name_key = "finetune_target"
        else:
            raise NotImplementedError("No target key in the original data")
        # if 'pdb_id' in sample[target_key].keys():
        #     name_key = 'pdb_id'
        if 'smi_name' in sample.keys():
            name_key = 'smi_name'
        else:
            raise NotImplementedError("No name key in the original data")
        ...
        logging_output = {
                "mol_repr_cls": encoder_rep[:, 0, :].data.cpu().numpy(),  # get cls token
                "pair_repr": encoder_pair_rep_list,
                "data_name": sample[name_key],
                "target": sample[target_key][target_name_key],
                "bsz": sample[input_key]["src_tokens"].size(0),
            }
        return 0, sample_size, logging_output
