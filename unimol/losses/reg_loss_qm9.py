...
@register_loss("finetune_mse_qm9")
class FinetuneMSELoss_qm9(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss = self.compute_loss(model, reg_output, sample, tgt_idx=self.args.tgt_idx, reduce=reduce)
        sample_size = sample["target"]["finetune_target"].size(0)
        if not self.training:
            if self.task.mean and self.task.std:
                targets_mean = torch.tensor(self.task.mean[self.args.tgt_idx], device=reg_output.device)
                targets_std = torch.tensor(self.task.std[self.args.tgt_idx], device=reg_output.device)
    ...
    def compute_loss(self, model, net_output, sample, tgt_idx, reduce=True):
        predicts = net_output.view(-1, self.args.num_classes).float()
        targets = (
            sample["target"]["finetune_target"].view(-1, self.args.num_classes).float()
        )
        if self.task.mean and self.task.std:
            targets_mean = torch.tensor(self.task.mean[tgt_idx], device=targets.device)
            targets_std = torch.tensor(self.task.std[tgt_idx], device=targets.device)
...
@register_loss("finetune_smooth_mae_qm9")
class FinetuneSmoothMAELoss_qm9(FinetuneMSELoss_qm9):
    def __init__(self, task):
        super().__init__(task)
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            metrics.log_scalar(f"{split}_agg_mae", agg_mae, sample_size, round=6)
    

# @register_loss("finetune_smooth_mae_qm9")
# class FinetuneSmoothMAELoss_qm9(UnicoreLoss):
#     def __init__(self, task):
#         super().__init__(task)
