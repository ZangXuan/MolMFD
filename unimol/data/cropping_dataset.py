...
class CroppingDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms, coordinates, shortest, bond_type, max_atoms=256):
        self.dataset = dataset
        self.seed = seed
        self.atoms = atoms
        self.coordinates = coordinates
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        atoms = dd[self.atoms]
        coordinates = dd[self.coordinates]
        shortest = dd[self.shortest]
        #bond_type = dd[self.bond_type]
        if self.max_atoms and len(atoms) > self.max_atoms:
            with data_utils.numpy_seed(self.seed, epoch, index):
                index = np.random.choice(len(atoms), self.max_atoms, replace=False)
                atoms = np.array(atoms)[index]
                coordinates = coordinates[index]
                shortest = shortest[np.ix_(index, index)]
                #bond_type = bond_type[np.ix_(index, index)]

        dd[self.atoms] = atoms
        dd[self.coordinates] = coordinates.astype(np.float32)
        dd[self.shortest] = shortest.astype(np.int64)
        # dd[self.bond_type] = bond_type.astype(np.int64)
        return dd
