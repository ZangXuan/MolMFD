...
class ConformerSampleDataset_qm9(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms, coordinates):
        self.dataset = dataset
        self.seed = seed
        self.atoms = atoms
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if "adjacency_matrix" in self.dataset[index].keys():
            adj = self.dataset[index]["adjacency_matrix"]
            return {"atoms": atoms, "coordinates": coordinates.astype(np.float32), "smi": smi, "adj": adj}
        else:
            return {"atoms": atoms, "coordinates": coordinates.astype(np.float32), "smi": smi}
