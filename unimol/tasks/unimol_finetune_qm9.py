...
from unimol.data import (
    KeyDataset,
    KeyDataset_qm9,
    ConformerSampleDataset_qm9,
    DistanceDataset,
    EdgeTypeDataset,
    RemoveHydrogenDataset_qm9,
    AtomTypeDataset,
    NormalizeDataset,
    CroppingDataset,
    RightPadDatasetCoord,
    data_utils,
    PrependTokenDataset2D,
    AppendTokenDataset2D
)
...
task_metainfo = {
    ...
    "qm9_3d": {
        ...
        "target_name": ['mu', 'alpha', 'homo', 'lumo', 'gap', 'r2', 'zpve', 'U0',
           'U', 'H', 'G', 'Cv'],
    },
    "qm9dft": {
    ...
    "qm9_0": {
        "mean": 2.6739518642425537,
        "std": 1.5078498125076294,
        "target_name": "mu",
    },
}


@register_task("mol_finetune_qm9")
class UniMolFinetuneTask_qm9(UnicoreTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=-1,
            type=int,
            help="-1: all target; >0: one target ",
        )
    ...
    def load_dataset(self, split, **kwargs):
        ...
        tgt_dataset = KeyDataset_qm9(dataset, "target", self.args.tgt_idx)
        smi_dataset = KeyDataset(dataset, "smi")
        #mol_dataset = KeyDataset(dataset, "mol")
        adj_dataset = KeyDataset(dataset, "adjacency_matrix")
        sample_dataset = ConformerSampleDataset_qm9(
            dataset, self.args.seed, "atoms", "coordinates"
        )
        dataset = AtomTypeDataset(dataset, sample_dataset)


        dataset = RemoveHydrogenDataset_qm9(
            dataset,
            "atoms",
            "coordinates",
            "smi",
        ...
        dataset = CroppingDataset(
            dataset, self.seed, "atoms", "coordinates", "shortest", "bond_type", self.args.max_atoms
        )
        ...
        def PrependAndAppend2D(dataset, pre_token, app_token):
            dataset = PrependTokenDataset2D(dataset, pre_token)
            return AppendTokenDataset2D(dataset, app_token)
        
        shortest_dataset = KeyDataset(dataset, "shortest")
        shortest_dataset = FromNumpyDataset(shortest_dataset)
        shortest_dataset = PrependAndAppend2D(shortest_dataset, 1, 1)
        # bond_type_dataset = KeyDataset(dataset, "bond_type")
        # bond_type_dataset = FromNumpyDataset(bond_type_dataset)
        # bond_type_dataset = PrependAndAppend2D(bond_type_dataset, 0, 1)
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                "net_input": {
                    ...
                    # "src_bond_type": RightPadDataset2D(
                    #     bond_type_dataset,
                    #     pad_idx=0,
                    # ),
