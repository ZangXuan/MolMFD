...
class UniMolTask(UnicoreTask):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        def one_dataset(raw_dataset, coord_seed, mask_seed):
            ...
            tokens_atomic_dataset = PrependAndAppend(
                token_dataset, self.dictionary.pad(), self.dictionary.pad()
            )
            ...
            def PrependAndAppend2D(dataset, pre_token, app_token, special_token=None):
                dataset = PrependTokenDataset2D(dataset, pre_token, special_token)
                return AppendTokenDataset2D(dataset, app_token, special_token)
            
            shortest_dataset = KeyDataset(dataset, "shortest")
            shortest_dataset = FromNumpyDataset(shortest_dataset)
            encoder_shortest_dataset = PrependAndAppend2D(shortest_dataset, 2, 2, 1)
            shortest_dataset = PrependAndAppend2D(shortest_dataset, 0, 0, 0)

            degree_dataset = KeyDataset(dataset, "degree")
            degree_dataset = FromNumpyDataset(degree_dataset)
            degree_dataset = PrependAndAppend(degree_dataset, 0, 0)
            ...
            return {
                ...
                "shortest_target": RightPadDataset2D(shortest_dataset, pad_idx=0),
                "coord_target": RightPadDatasetCoord(coord_dataset, pad_idx=0),
                "smi_name": RawArrayDataset(smi_dataset),
                "tokens_atomic": RightPadDataset(tokens_atomic_dataset, pad_idx=self.dictionary.pad()),
                "degree_target":RightPadDataset(degree_dataset, pad_idx=0)
            }
